...
//  Serial.println("GET https://api.thingspeak.com/update?api_key=6FU7YFXWVWCNCECH&field1=" + String(Distance_S));
  gprsSerial.println("GET https://api.thingspeak.com/update?api_key=3MOS749RQ01VJOEN&field1=" + String(Distance_S));//begin send data to remote server
  delay(8000);
  ShowSerialData();
...
//  for (int i = 0; i<11 ; i++) reading = analogRead(forcePin);
  Serial.println(reading);
   while (reading > 500) {
    //Serial.print("lowering bob down Reading : ");
...
//    delay(500);
  ...
  while (reading < 500) {
    //Serial.print("Pulling bob up, Reading : ");
    myStepper.step(-162);
//    delay(500);
    distance -= stepval * dist_per_step;
